const CONFIG = {
  ...
  xiaozhiTtsTimeout: 7000,

  // MIMI PRO WEB → MIMI AI Core → TTS Bridge.
  // Primary TTS path for the deployed GitHub Pages version.
...
function getFastResponse(text) {
  ...
  async function processUserText(finalText) {
    ...
    const pushTtsChunk = (chunk, flush = false) => {
      ...
      // Nếu câu dài, cắt mềm để TTS không phải đợi quá lâu.
      while (ttsBuffer.length >= 90) {
        const cut = ttsBuffer.lastIndexOf(" ", 90);
        const index = cut > 30 ? cut : 90;
  ...
  async function processUserText(finalText) {
    ...
    const runTtsQueue = async () => {
      ...
      try {
        while (ttsQueue.length) {
          const chunk = ttsQueue.shift();
          const ok = await speakWithMimiWorkerTts(chunk) || await speakWithXiaozhi(chunk);
          if (!ok) {
            ttsFailed = true;
            // Fallback browser TTS for the remaining stream text.
            speak(chunk);
    ...
    const pushTtsChunk = (chunk, flush = false) => {
      ...
      // Nếu câu dài, cắt mềm để TTS không phải đợi quá lâu.
      while (ttsBuffer.length >= 120) {
        const cut = ttsBuffer.lastIndexOf(" ", 120);
        const index = cut > 40 ? cut : 120;
        const value = ttsBuffer.slice(0, index).trim();
        ttsBuffer = ttsBuffer.slice(index).trimStart();
        if (value) ttsQueue.push(value);
      }
